    ...
    <section class="content">
        <div class="box box-info">
            <div class="box-header">
                <h4 style="text-align:center"><b>DATA SURAT MENIKAH</b></h4>
                <hr>
            </div>
            ...
            <div class="box-body table-responsive">
                ...
                <p>
                    <a href="<?= base_url('menikah/tambah'); ?>" class="btn btn-primary">Tambah SK Menikah</a>
                </p>
                <table id="data" class="table table-striped table-bordered" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        <?php
                        $no = 1;
                        foreach ($menikah as $mnkh) {
                        ?>
                            <tr>
                                <td style="text-align:center"><?= $no; ?></td>
                                <td><?= $mnkh->nik; ?></td>
                                <td><?= $mnkh->nama; ?></td>
                                <td><?= $mnkh->nama_pejabat; ?></td>
                                <td style="text-align:center">
                                    <a href="<?= base_url('menikah/edit/' . $mnkh->id_menikah); ?>" class="btn btn-success btn-xs"><i class="fa fa-edit"></i> Edit</a>
                                    <a href="#" class="btn btn-danger btn-xs" onclick="showDeleteConfirmationModal('<?= base_url('menikah/hapus/' . $mnkh->id_menikah); ?>')"><i class="fa fa-trash-o"></i>Hapus</a>
                                    <button onclick="printSktm('<?= $mnkh->id_menikah; ?>')" class="btn btn-info btn-xs"><i class="fa fa-print"></i> Cetak</button>
                            </tr>
                            </td>
                        <?php
                            $no++;
...
<script>
    ...
    function printSktm(id) {
        ...
        window.open('<?= base_url('menikah/cetak/'); ?>' + id, '_self');
    }
