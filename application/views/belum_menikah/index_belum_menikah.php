    ...
    <section class="content">
        <div class="box box-info">
            <div class="box-header">
                <h4 style="text-align:center"><b>DATA SURAT KETERANGAN BELUM MENIKAH</b></h4>
                <hr>
            </div>
            ...
            <div class="box-body table-responsive">
                ...
                <p>
                    <a href="<?= base_url('belum_menikah/tambah'); ?>" class="btn btn-primary">Tambah SK Belum Menikah</a>
                </p>
                <table id="data" class="table table-striped table-bordered" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        <?php
                        $no = 1;
                        foreach ($belum_menikah as $bm) {
                        ?>
                            <tr>
                                <td style="text-align:center"><?= $no; ?></td>
                                <td><?= $bm->nik; ?></td>
                                <td><?= $bm->nama; ?></td>
                                <td><?= $bm->nama_pejabat; ?></td>
                                <td style="text-align:center">
                                    <a href="<?= base_url('belum_menikah/edit/' . $bm->id_belum_menikah); ?>" class="btn btn-success btn-xs"><i class="fa fa-edit"></i> Edit</a>
                                    <a href="#" class="btn btn-danger btn-xs" onclick="showDeleteConfirmationModal('<?= base_url('belum_menikah/hapus/' . $bm->id_belum_menikah); ?>')"><i class="fa fa-trash-o"></i>Hapus</a>
                                    <button onclick="printSktm('<?= $bm->id_belum_menikah; ?>')" class="btn btn-info btn-xs"><i class="fa fa-print"></i> Cetak</button>
                            </tr>
                            </td>
                        <?php
                            $no++;
...
<script>
    ...
    function printSktm(id) {
        ...
        window.open('<?= base_url('belum_menikah/cetak/'); ?>' + id, '_self');
    }
